# List of Origins & Abilities

## Origins-Reborn

### Avian

`avian` · (٭)

The Avian race has lost their ability to fly a long time ago. Now these peaceful creatures can be seen gliding from one place to another.

**Ablities:**: 

- Featherweight: You fall as gently to the ground as a feather would, unless you sneak.
- Fresh Air: When sleeping, your bed needs to be at an altitude of at least %s blocks, so you can breathe fresh air.
- Tailwind: You are a little bit quicker on foot than others.
- Oviparous: Whenever you wake up in the morning, you will lay an egg.
- Vegetarian: You can't digest any meat.
- `origins:like_air` (Unknown ability)

### Feline

`feline` · (٭٭)

With their cat-like appearance, the Feline scare creepers away. With the dexterity of cats, they always land safely on their feet.

**Ablities:**: 

- Acrobatics: You never take fall damage, no matter from which height you fall.
- Strong Ankles: You are able to jump higher by jumping while sprinting.
- Velvet Paws: Your footsteps don't cause any vibrations which could otherwise be picked up by nearby lifeforms.
- Nine Lives: You have 1 less heart of health than humans.
- Weak Arms: When not under the effect of a strength potion, you can only mine natural stone if there are at most 2 other natural stone blocks adjacent to it.
- Catlike Appearance: Creepers are scared of you and will only explode if you attack them first.
- Nocturnal: You can slightly see in the dark when not in water.

### Human

`human`

A regular human. Your ordinary Minecraft experience awaits.

**Ablities:**: 


### Merling

`merling` · (٭٭٭)

These natural inhabitants of the ocean are not used to being out of the water for too long.

**Ablities:**: 

- Gills: You can breathe underwater, but not on land.
- Wet Eyes: Your vision underwater is perfect.
- Aqua Affinity: You may break blocks underwater as others do on land.
- Fins: Your underwater speed is increased.
- Like Water: When underwater, you do not sink to the ground unless you want to.
- `origins:aquatic` (Unknown ability)
- `origins:conduit_power_on_land` (Unknown ability)
- `origins:air_from_potions` (Unknown ability)

### Arachnid

`arachnid` · (٭)

Their climbing abilities and the ability to trap their foes in spiderweb make the Arachnid perfect hunters.

**Ablities:**: 

- Climbing: You are able to climb up any kind of wall, not just ladders.
- Master of Webs: You navigate cobweb perfectly, and are able to climb in them. When you hit an enemy in melee, they get stuck in cobweb for a while. Non-arthropods stuck in cobweb will be sensed by you. You are able to craft cobweb from string.
- Carnivore: Your diet is restricted to meat, you can't eat vegetables.
- Fragile: You have 3 less hearts of health than humans.
- `origins:arthropod` (Unknown ability)
- Dark Eyes: You can see in the dark after generations of evolution.
- Stinger: When you punch someone with your fist, you poison them for a few seconds.

### Blazeborn

`blazeborn` · (٭٭٭)

Late descendants of the Blaze, the Blazeborn are naturally immune to the perils of the Nether.

**Ablities:**: 

- Fire Immunity: You are immune to all types of fire damage.
- Nether Inhabitant: Your natural spawn will be in the Nether.
- Burning Wrath: When on fire, you deal additional damage with your attacks.
- Hotblooded: Due to your hot body, venoms burn up, making you immune to poison and hunger status effects.
- Hydrophobia: You receive damage over time while in contact with water.
- `origins:flame_particles` (Unknown ability)
- `origins:damage_from_snowballs` (Unknown ability)
- `origins:damage_from_potions` (Unknown ability)

### Enderian

`enderian` · (٭٭)

Born as children of the Ender Dragon, Enderians are capable of teleporting but are vulnerable to water.

**Ablities:**: 

- Teleportation: Whenever you want, you may throw an ender pearl which deals no damage, allowing you to teleport.
- Hydrophobia: You receive damage over time while in contact with water.
- Scared of Gourds: You are afraid of pumpkins. For a good reason.
- Slender Body: You can reach blocks and entities further away.
- `origins:ender_particles` (Unknown ability)
- `origins:damage_from_potions` (Unknown ability)

### Phantom

`phantom` · (٭٭٭)

As half-human and half-phantom beings, these creatures can switch between a Phantom and a normal form.

**Ablities:**: 

- `origins:phantomize` (Unknown ability)
- Phasing: While phantomized, you can walk through solid material, except Obsidian.
- Invisibility: While phantomized, you are invisible.
- Photoallergic: You begin to burn in daylight if you are not invisible.
- Fast Metabolism: Being phantomized causes you to become hungry.
- Fragile: You have 3 less hearts of health than humans.
- `origins:translucent` (Unknown ability)
- `origins:phantomize_overlay` (Unknown ability)

### Elytrian

`elytrian` · (٭)

Often flying around in the winds, Elytrians are uncomfortable when they don't have enough space above their head.

**Ablities:**: 

- Winged: You have Elytra wings without needing to equip any.
- Gift of the Winds: Every 30 seconds, you are able to launch about 20 blocks up into the air.
- Aerial Combatant: You deal substantially more damage while in Elytra flight.
- Need for Mobility: You can not wear any heavy armor (armor with protection values higher than chainmail).
- Claustrophobia: Being somewhere with a low ceiling for too long will weaken you and make you slower.
- Brittle Bones: You take more damage from falling and flying into blocks.

### Shulk

`shulk` · (٭)

Related to Shulkers, the bodies of the Shulk are outfitted with a protective shell-like skin.

**Ablities:**: 

- Hoarder: You have access to an additional 9 slots of inventory, which keep the items on death.
- Sturdy Skin: Even without wearing armor, your skin provides natural protection.
- Strong Arms: You are strong enough to break natural stones without using a pickaxe.
- Unwieldy: The way your hands are formed provide no way of holding a shield upright.
- Large Appetite: You exhaust much quicker than others, thus requiring you to eat more.

### Rat

`1rat` · (٭٭٭)

Small and sneaky rodents that thrive in urban environments.

**Ablities:**: 

- Small Body: You're only one block tall, half as tall as a regular human.
- Large Appetite: You exhaust much quicker than others, thus requiring you to eat more.
- Dark Eyes: You can see in the dark after generations of evolution.
- Sly: You are quicker.
- Fast Miner: You're well trained in mining, so are much faster than a regular human.
- Brittle Bones: You take more damage from falling and flying into blocks.
- Strong Ankles: You are able to jump higher by jumping while sprinting.
- Afraid of Cats: You get nausea and weakness when around cats.
- Poo: You will drop cocoa beans (poo) when sneaking
- Fragile: You have 3 less hearts of health than humans.
- Vegetarian: You can't digest any meat.

### Breeze

`2breeze` · (٭)

A breeze-like being that can manipulate wind to boost themselves into the air.

**Ablities:**: 

- Wind Jump: You release a wind charge when sneaking, allowing you to boost yourself into the air.
- Acrobatics: You never take fall damage, no matter from which height you fall.
- Unwieldy: The way your hands are formed provide no way of holding a shield upright.
- Tailwind: You are a little bit quicker on foot than others.
- `origins:damage_from_potions` (Unknown ability)
- Thick Skin: You've got naturally thicker skin than regular humans, so arrows do a lot less damage.
- Small Fox: You have 2 less hearts of health than humans.

### Frog

`3frog` · (٭٭)

Amphibians, able to move in both water and land, as long as they don't dry out.

**Ablities:**: 

- Acrobatics: You never take fall damage, no matter from which height you fall.
- Strong Ankles: You are able to jump higher by jumping while sprinting.
- Unwieldy: The way your hands are formed provide no way of holding a shield upright.
- Wet Eyes: Your vision underwater is perfect.
- Warm Body: Your warm body conflicts with colder biomes, slowing you down.
- Overheat: You have a temperature bar that slowly begins to fill in hot biomes, and cool in other biomes.
- Cold Body: Your cold body conflicts with warmer biomes, slowing you down.
- `monsterorigins:water_breathing` (Unknown ability)
- Frog Leap: Double left-clicking while sneaking allows you to leap into the air.
- Frozen Skin: You take double damage from all sources of fire.
- Tailwind: You are a little bit quicker on foot than others.
- Surface Weakness: You are weakened while on land.
- `moborigins:temperature` (Unknown ability)

### Kraken

`4kraken` · (٭٭٭)

A large and mighty sea beast that lurks in the ocean depths

**Ablities:**: 

- Gills: You can breathe underwater, but not on land.
- Wet Eyes: Your vision underwater is perfect.
- Water Based: You're used to the water, so move much slower on land.
- Aqua Affinity: You may break blocks underwater as others do on land.
- Like Water: When underwater, you do not sink to the ground unless you want to.
- Photoallergic: You begin to burn in daylight if you are not invisible.
- Confusion: Right clicking on a player will give them Nausea for 30 seconds.
- Carnivore: Your diet is restricted to meat, you can't eat vegetables.
- Large Appetite: You exhaust much quicker than others, thus requiring you to eat more.
- Sturdy Skin: Even without wearing armor, your skin provides natural protection.
- Telekinetic Reach: You can reach things much further away than normal.
- Large Body: You're three blocks tall, taller than a regular human.
- Iron Stomach: You have an immunity to poison and harming potion effects.
- Ocean Wish: Your natural habitat is the ocean, so you're much weaker when you're not in the water.
- Call of the Sea: You can call a thunderstorm on command using /cots every so often.

### Fat Chud

`5chud` · (٭٭٭٭٭)

A loser with almost every debuff in game.

**Ablities:**: 

- Claustrophobia: Being somewhere with a low ceiling for too long will weaken you and make you slower.
- Carnivore: Your diet is restricted to meat, you can't eat vegetables.
- Unwieldy: The way your hands are formed provide no way of holding a shield upright.
- Photoallergic: You begin to burn in daylight if you are not invisible.
- `origins:damage_from_potions` (Unknown ability)
- `origins:wa` (Unknown ability)
- `origins:grayscale` (Unknown ability)
- Fragile: You have 3 less hearts of health than humans.
- Need for Mobility: You can not wear any heavy armor (armor with protection values higher than chainmail).
- Large Appetite: You exhaust much quicker than others, thus requiring you to eat more.
- Brittle Bones: You take more damage from falling and flying into blocks.
- Scared of Gourds: You are afraid of pumpkins. For a good reason.
- Vegetarian: You can't digest any meat.
- Hydrophobia: You receive damage over time while in contact with water.
- Weak Arms: When not under the effect of a strength potion, you can only mine natural stone if there are at most 2 other natural stone blocks adjacent to it.
- `origins:arthropod` (Unknown ability)
- `origins:aquatic` (Unknown ability)
- `origins:damage_from_potions` (Unknown ability)
- `origins:damage_from_snowballs` (Unknown ability)
- Blindness: You can't see anything further than a few blocks away, though you can see further with night vision.
- Warm Body: Your warm body conflicts with colder biomes, slowing you down.
- Frozen Skin: You take double damage from all sources of fire.
- Afraid of Cats: You get nausea and weakness when around cats.
- Poor Digestion: You can only hold half as much saturation as a human.
- Cold Body: Your cold body conflicts with warmer biomes, slowing you down.
- Water Based: You're used to the water, so move much slower on land.
- Terrifying Monster: Villagers are scared of you and refuse to trade with you.
- Zombie Slowness: Your undead body moves at a slower pace than humans.
- Zombie Hunger: Your constant hunger for flesh makes you exhaust quicker than a human.
- `monsterorigins:undead` (Unknown ability)
- Bouncy: All blocks act like slime blocks.
- Melting: As your temperature bar fills up, you'll slowly begin to melt in hot biomes, losing health and speed.
- Overheat: You have a temperature bar that slowly begins to fill in hot biomes, and cool in other biomes.
- Split Ability: Turn your food points into a small slime to defend you!
- Surface Weakness: You are weakened while on land.
- Undead: You are undead, and burn in the daylight. You also take more damage from smite.
- `moborigins:melting_speed` (Unknown ability)
- `moborigins:surface_slowness` (Unknown ability)
- `moborigins:temperature` (Unknown ability)
- Daylight Sensitive: Your greatest weakness is daylight, which causes you to burst into flames.
- Large Body: You're three blocks tall, taller than a regular human.
- Clumsy Shot: Your big hands are clumsy with a bow, so your arrows are slow and not very accurate.
- Water Sensitive: You are hurt by water as its current drains your power.
- Unskilled: Your reliance on your magic has left you unskilled at melee combat.
- Hydrophobia: Water saps your energy, leaving you with only 3 hearts of health.

### Flea

`6flea` · (٭٭٭)

Bloodsucking, lifestealing bug that is an expert at hiding away and surviving

**Ablities:**: 

- Small Body: You're only one block tall, half as tall as a regular human.
- Carnivore: Your diet is restricted to meat, you can't eat vegetables.
- Fragile: You have 3 less hearts of health than humans.
- Dark Magic: Whenever you kill something, you absorb some of its health for up to 30 extra hearts.
- Sly: You are quicker.
- Weak Arms: When not under the effect of a strength potion, you can only mine natural stone if there are at most 2 other natural stone blocks adjacent to it.
- Acrobatics: You never take fall damage, no matter from which height you fall.
- Unwieldy: The way your hands are formed provide no way of holding a shield upright.
- Small Bug: You have 2 less hearts of health than humans.
- `origins:arthropod` (Unknown ability)
- Hydrophobia: You receive damage over time while in contact with water.
- Confusion: Right clicking on a player will give them Nausea for 30 seconds.
- Strong Ankles: You are able to jump higher by jumping while sprinting.

### Armadillo

`7armadillo` · (٭)

A defensive mob that is almost impossible to kill.

**Ablities:**: 

- Unskilled: Your reliance on your magic has left you unskilled at melee combat.
- Zombie Slowness: Your undead body moves at a slower pace than humans.
- Curl Up: You can curl up into a ball whilst sneaking, giving you resistance and slowness
- Clumsy Shot: Your big hands are clumsy with a bow, so your arrows are slow and not very accurate.
- Natural Armor: Your tough and strong body gives you some natural defense against attacks.
- Tough Skin: You have double the health of a regular human.

### Flying Fish

`8flyingfish` · (٭٭٭)

A fish with wings, able to glide under and above water with grace

**Ablities:**: 

- Winged: You have Elytra wings without needing to equip any.
- Fins: Your underwater speed is increased.
- Gills: You can breathe underwater, but not on land.
- Fragile: You have 3 less hearts of health than humans.
- Unwieldy: The way your hands are formed provide no way of holding a shield upright.
- `origins:aquatic` (Unknown ability)
- `origins:air_from_potions` (Unknown ability)
- Gift of the Winds: Every 30 seconds, you are able to launch about 20 blocks up into the air.
- Weak Arms: When not under the effect of a strength potion, you can only mine natural stone if there are at most 2 other natural stone blocks adjacent to it.
- Wet Eyes: Your vision underwater is perfect.
- Aqua Affinity: You may break blocks underwater as others do on land.
- Unskilled: Your reliance on your magic has left you unskilled at melee combat.
- Ocean's Grace: You are a part of the water, so you have extra health and deal extra damage when in water or rain.
- Ocean Wish: Your natural habitat is the ocean, so you're much weaker when you're not in the water.

## Origins-Fantasy

### Centaur

`centaur` · (٭٭٭)

Worshippers of the moon and trained fighters, the Centaurs are much more powerful than regular horses, and become stronger on a full moon.

**Ablities:**: 

- Half Horse: You are half horse, half human.
- Powerful Jump: You've trained for your whole life, so can jump much higher than a regular horse.
- Dashmaster: From years of training for race after race, you're much faster than any normal horse.
- Moon's Blessing: You're a worshipper of the moon, so on nights with a full moon you're stronger than normal.
- Skilled Archer: You've trained with a bow for many years, and you shoot your arrows perfectly straight.

### Elf

`elf` · (٭)

Skilled masters of the bow, Elves shoot arrows that are much stronger and faster than any other creatures.

**Ablities:**: 

- Skilled Archer: You've trained with a bow for many years, and you shoot your arrows perfectly straight.
- Piercing Shot: All arrows you shoot deal increased damage.
- Masterful Speed: Arrows you shoot are twice as fast than ones shot by a regular human.
- Arrow Lord: Your connection to your bow and arrow enhances any potion effects placed on your arrows.
- Bow Burst: By casting a spell on any regular arrow, you can instantly shoot %s arrows at once using only one, but this disables your bow for %s seconds.

### Giant

`giant` · (٭٭)

Although clumsy and weak with a bow and arrow, Giants are powerful creatures that can destroy any foe with their fists.

**Ablities:**: 

- Double Health: As you're larger than humans, you have more health as your body protects you from damage.
- Heavy Blow: Your attacks are stronger than humans, but you have a longer attack cooldown.
- Clumsy Shot: Your big hands are clumsy with a bow, so your arrows are slow and not very accurate.
- Large Body: You're three blocks tall, taller than a regular human.
- Zombie Slowness: Your undead body moves at a slower pace than humans.

### Human

`human`

A regular human, born without any special abilities.

**Ablities:**: 


### Bard

`bard` · (٭٭٭)

Bards are masters of music and song, and emit a musical aura.

**Ablities:**: 

- Elegy: You become stronger when at less than 3 hearts.
- Musically Attuned: You gain strength and speed when a nearby Note Block is played.
- Allay Master: Your musical aura allows you to breed allays without playing music.
- Chime: You can absorb the chime of amethyst shards to regenerate health.
- Bardic Intuition: Your musical energy will sometimes cause a creeper to drop a music disc, even without a skeleton.

### Dragonborn

`dragonborn` · (٭)

Children of ancient, long forgotten dragons, the Dragonborn have the increased power of their ancestors when in their homeland of the End.

**Ablities:**: 

- Dragon's Breath: By right clicking using an empty bottle, you can store your own Dragon's Breath.
- End Inhabitant: Your natural habitat is the end, so you have more health and are stronger when you are there.
- Crystal Healer: You can regenerate health from nearby End Crystals.
- Dragon's Fireball: You can right click whilst holding a sword to launch a dragon's fireball, with a cooldown of %s seconds.
- Thick Skin: You've got naturally thicker skin than regular humans, so arrows do a lot less damage.

### Dwarf

`dwarf` · (٭٭)

Known for their great skill in finding minerals, the Dwarfs spend most of their time in caves deep underground.

**Ablities:**: 

- Fast Miner: You're well trained in mining, so are much faster than a regular human.
- Dark Eyes: You can see in the dark after generations of evolution.
- Careful Miner: Your care and mastery in the art of extracting minerals results in a much higher yield from ores than other creatures.
- Iron Stomach: You have an immunity to poison and harming potion effects.
- Small Body: You're only one block tall, half as tall as a regular human.

### Gargoyle

`gargoyle` · (٭٭)

Stone statues awoken by magic, Gargoyles move slowly but are very strong.

**Ablities:**: 

- `fantasyorigins:stone_skin` (Unknown ability)
- Solid Stance: You are immune to knockback.
- Stiff: Your stone body makes it harder to move, causing you to walk slower.
- Heat Resistant: Your stone skin makes you immune to fire.
- Potion Resistant: Potions have no effect on you as your body is made of stone.
- Heavy Blow: Your attacks are stronger than humans, but you have a longer attack cooldown.
- Sturdy Skin: Even without wearing armor, your skin provides natural protection.

### Naiad

`naiad` · (٭٭٭)

Spirits that have a special connection to the water, Naiads become much stronger in their home territory, though are weak on land.

**Ablities:**: 

- Ocean's Grace: You are a part of the water, so you have extra health and deal extra damage when in water or rain.
- Ocean Wish: Your natural habitat is the ocean, so you're much weaker when you're not in the water.
- Gills: You can breathe underwater, but not on land.
- Wet Eyes: Your vision underwater is perfect.
- Aqua Affinity: You may break blocks underwater as others do on land.
- Fins: Your underwater speed is increased.
- Like Water: When underwater, you do not sink to the ground unless you want to.

### Vampire

`vampire` · (٭٭٭)

The vampire is a powerful creature of the night, with the ability to transform those it kills into more of its kind.

**Ablities:**: 

- Vampiric Transformation: You can transform other players into vampires by killing them.
- Daylight Sensitive: Your greatest weakness is daylight, which causes you to burst into flames.
- Water Sensitive: You are hurt by water as its current drains your power.
- Leeching: Upon killing a mob or player, you sap a portion of its health, healing you.
- Stronger: Your vampiric nature makes you stronger than a regular human, making your physical attacks deal far more damage.
- Undead Ally: As an undead monster, other undead creatures will not attack you unprovoked.

## Origins-Magic

### Alchemist

`alchemist` · (٭)

The alchemist knows its way around potions, as well as being able to produce gold.

**Ablities:**: 

- Potion Master: Potions you drink and throw are much stronger.
- Alchemy: Right clicking on a Copper Block will turn it to Gold, and vice versa.
- Unskilled: Your reliance on your magic has left you unskilled at melee combat.
- Hydrophobia: You receive damage over time while in contact with water.
- `magicorigins:poor_shot` (Unknown ability)
- Iron Stomach: You have an immunity to poison and harming potion effects.

### Healer

`healer` · (٭٭)

Healers try to avoid fights and instead heal others, working to keep their teammates safe.

**Ablities:**: 

- Final Shout: You can survive death once every 10 minutes.
- Healing Touch: Right clicking on something will give it Regeneration II for 10 seconds.
- Healing Focus: When standing still you focus your energy to regenerate health.
- Unskilled: Your reliance on your magic has left you unskilled at melee combat.
- Weak Arms: When not under the effect of a strength potion, you can only mine natural stone if there are at most 2 other natural stone blocks adjacent to it.

### Warlock

`warlock` · (٭٭٭)

Evil sorcerers using corrupted magic, Warlocks cannot use regular magic but have their own, stronger powers.

**Ablities:**: 

- Dark Magic: Whenever you kill something, you absorb some of its health for up to 30 extra hearts.
- Cursed Power: Upon hitting something, it gains a negative effect for 15 seconds.
- Undead Ally: With your power over darkness, undead creatures will not attack you unprovoked.
- Dark Aura: You emit a dark aura that makes villagers afraid of you.
- Hydrophobia: You receive damage over time while in contact with water.
- Water Sensitive: You are hurt by water as its current drains your power.
- Hydrophobia: Water saps your energy, leaving you with only 3 hearts of health.
- Magic Resistant: Your dark magic repels regular forms of magic like potions and enchantments.

### Hypnotist

`hypnotist` · (٭٭)

Hypnotists have a knack for confusing and controlling their enemies to get their way.

**Ablities:**: 

- Hypnosis: Right clicking a monster will hypnotise it to target the last other thing you attacked.
- Confusion: Right clicking on a player will give them Nausea for 30 seconds.
- Unskilled: Your reliance on your magic has left you unskilled at melee combat.
- Bartering Master: You're brilliant at bartering after a lifetime of experience, every time you barter you get between 2 and 5 times as many valuables.
- `moborigins:illager` (Unknown ability)

### Shadowmancer

`shadowmancer` · (٭٭٭)

Shadowmancers push themselves to their very limits, giving them fearsome and dark power.

**Ablities:**: 

- Shadow Form: In really dark places you enter Shadow Form, where nothing see or attack you.
- Dark Strength: You deal more damage in the dark, but less in the light.
- `magicorigins:double_fire_damage` (Unknown ability)
- Creature of Darkness: You catch fire in the presence of light, even in small amounts.
- `magicorigins:no_fire_resistance` (Unknown ability)

### Telekinetic

`telekinetic` · (٭)

Mages who use their minds to extend their reach and pick up items from a distance.

**Ablities:**: 

- Telekinetic Reach: You can reach things much further away than normal.
- Telekinesis: Items dropped from blocks and entities go straight into your inventory.
- Slender Body: You can reach blocks and entities further away.
- `moborigins:small_weak_knockback` (Unknown ability)
- Brittle Bones: You take more damage from falling and flying into blocks.
- Claustrophobia: Being somewhere with a low ceiling for too long will weaken you and make you slower.

### Necromancer

`necromancer` · (٭٭)

Dark mages who use spells to resurrect and control the dead.

**Ablities:**: 

- Lord of the Dead: Nearby undead monsters not targeting you will go after whatever you attack.
- Resurrection Spell: When you swing your fist, nearby dead players that have not yet respawned come back where they died.
- Spirit Strength: You are stronger in the Nether, but weaker outside of it.
- Undead Ally: With your power over darkness, undead creatures will not attack you unprovoked.
- Dark Aura: You emit a dark aura that makes villagers afraid of you.
- `monsterorigins:wither_immunity` (Unknown ability)
- Nether Inhabitant: Your natural spawn will be in the Nether.
- `monsterorigins:undead` (Unknown ability)
- Pillager Aligned: Villagers don't like you, and pillagers like you!
- Leeching: Upon killing a mob or player, you sap a portion of its health, healing you.
- Hydrophobia: You receive damage over time while in contact with water.

### Spirit

`spirit` · (٭٭)

Spirits are lost souls that continue to wander the earth, rarely sighted and heard even less.

**Ablities:**: 

- Spectral: Your ghost-like nature prevents you being detected by Sculk Sensors when you move or eat.
- Ghostly: You can stand so still that you can't be seen.
- Spirit Strength: You are stronger in the Nether, but weaker outside of it.
- Poor Digestion: You can only hold half as much saturation as a human.

## Origins-Mobs

### Drowned

`drowned` · (٭)

The dwellers of the deep ocean, they wield their tridents and attack the living that dare cross the ocean.

**Ablities:**: 

- Trident Expert: You're a master of the trident, dealing +2 damage when you throw it, and +2 melee damage with it. You can also use channeling without thunder, and use riptide without rain/water at the price of extra durability.
- `moborigins:water_vision` (Unknown ability)
- Guardian Ally: Guardians don't attack you!
- Water Combatant: You deal more damage while in water.
- `moborigins:water_breathing` (Unknown ability)
- Undead: You are undead, and burn in the daylight. You also take more damage from smite.
- Zombie Hunger: Your constant hunger for flesh makes you exhaust quicker than a human.
- `moborigins:surface_slowness` (Unknown ability)
- Carnivore: Your diet is restricted to meat, you can't eat vegetables.

### Fox

`fox` · (٭)

Sly and cunning, foxes are timid animals who enjoy eating berries, and stealing items.

**Ablities:**: 

- Timid Creature: Your speed increases when you are around more than 3 other players.
- Better Berries: Berries taste extra delicious to you!
- Careful Gatherer: Sweet Berry Bushes don't hurt you at all.
- Item Collector: You have a larger item pickup radius.
- Sly: You are quicker.
- Small Fox: You have 2 less hearts of health than humans.

### Wolf

`wolf` · (٭)

Strong and Fast, Wolves are creatures that have strength in numbers.

**Ablities:**: 

- Wolf Body: You have 2 less hearts of health than humans.
- Werewolf-like: During a full moon you get Faster, Stronger, and Healthier.
- `moborigins:full_moon_health` (Unknown ability)
- `moborigins:full_moon_attack` (Unknown ability)
- Alpha Wolf: Wolves you tame are stronger!
- Howl: You can use the left click key when holding nothing to howl, and give speed and strength to nearby wolves and yourself.
- Wolf Pack: When you are near at least 4 wolves you gain speed and attack damage.
- `moborigins:wolf_pack_attack` (Unknown ability)
- Carnivore: Your diet is restricted to meat, you can't eat vegetables.

### Slime

`slime` · (٭)

Green blobs of Jello, that bounce all around!

**Ablities:**: 

- Bouncy: All blocks act like slime blocks.
- Split Ability: Turn your food points into a small slime to defend you!
- Small Weakness: When at less than 2 hearts, you deal almost no damage, but your attacks have stronger knockback!
- `moborigins:small_weak_knockback` (Unknown ability)
- Acrobatics: You never take fall damage, no matter from which height you fall.
- `moborigins:slimy_color` (Unknown ability)
- `moborigins:dyeable` (Unknown ability)

### Bee

`bee` · (٭٭)

A flying insect that pollinates the world, spreading flowers and honey all around.

**Ablities:**: 

- Stinger: When you punch someone with your fist, you poison them for a few seconds.
- Flower Power: When near multiple flowers, you gain regeneration.
- Queen Bee: When you collect honey, the bees won't try to attack you.
- Small Bug: You have 2 less hearts of health than humans.
- Bee Wings: You can use your tiny bee wings to descend slower as an ability.
- Vegetarian: You can't digest any meat.

### Evoker

`evoker` · (٭٭)

Master of Magic, and a collector of Totems.

**Ablities:**: 

- Summon Fangs: You have the ability to summon fangs!
- Arcane Totems: Totems have a %s% chance not to break on use.
- Pillager Aligned: Villagers don't like you, and pillagers like you!
- `moborigins:illager` (Unknown ability)

### Snow Golem

`snow_golem` · (٭٭٭)

Golems made from snow, and weak to anything that might melt them.

**Ablities:**: 

- Hydrophobia: You receive damage over time while in contact with water.
- `moborigins:temperature` (Unknown ability)
- Overheat: You have a temperature bar that slowly begins to fill in hot biomes, and cool in other biomes.
- Melting: As your temperature bar fills up, you'll slowly begin to melt in hot biomes, losing health and speed.
- `moborigins:melting_speed` (Unknown ability)
- Stronger Snowballs: Snowballs you throw are packed with ice, and deal 1 damage!
- Snow Trail: You leave a trail of snow.
- Frigid Strength: Deal more damage in cold areas.

### Strider

`strider` · (٭٭٭)

Travel the vast oceans of lava with the speed of a strider.

**Ablities:**: 

- Fire Immunity: You are immune to all types of fire damage.
- Nether Inhabitant: Your natural spawn will be in the Nether.
- Hydrophobia: You receive damage over time while in contact with water.
- Lava Walker: You have the ability to walk on lava source blocks! You are also quicker while walking on lava, and slower on land.
- `moborigins:surface_slowness` (Unknown ability)
- Rideable Creature: Other players can ride you!
- `moborigins:strider_shake` (Unknown ability)
- Fungus Hunger: You can eat warped fungus to recover some hunger, along with a small speed boost.

### Witch

`witch` · (٭٭)

Cruel and cackling, Witches are masters of brewing, and are great with magic.

**Ablities:**: 

- Better Potions: You consume potions better than most, Potions will last longer when you drink them.
- Perfect Potion: Get a random potion effect, based on the situation you are in.
- `moborigins:witch_particles` (Unknown ability)
- Pillager Aligned: Villagers don't like you, and pillagers like you!
- `moborigins:illager` (Unknown ability)

### Elder Guardian

`elder_guardian` · **UNCHOOSABLE** · (٭٭٭)

Elder Guardians are the leaders of monuments, and wield more power than normal guardians.

**Ablities:**: 

- Elder Spikes: Spikes that have a chance to damage attackers!
- Elder Magic: You can cast a spell on nearby players to slow down their mining speed.
- `moborigins:mining_fatigue_immune` (Unknown ability)
- Guardian Ally: Guardians don't attack you!
- Prismarine Skin: Your skin is made of prismarine, and you get natural armor from it.
- Surface Weakness: You are weakened while on land.
- Gills: You can breathe underwater, but not on land.
- Wet Eyes: Your vision underwater is perfect.
- Aqua Affinity: You may break blocks underwater as others do on land.
- Fins: Your underwater speed is increased.
- Like Water: When underwater, you do not sink to the ground unless you want to.
- `origins:aquatic` (Unknown ability)
- `origins:conduit_power_on_land` (Unknown ability)
- `origins:air_from_potions` (Unknown ability)

### Guardian

`guardian` · (٭٭٭)

Guardians are known for protecting their Monuments and the treasured sponges within. Some are known for evolving into Elder Guardians.

**Ablities:**: 

- Guardian Spikes: Spikes that have a chance to damage attackers!
- Guardian Ally: Guardians don't attack you!
- Prismarine Skin: Your skin is made of prismarine, and you get natural armor from it.
- Surface Weakness: You are weakened while on land.
- `moborigins:becomes_elder_guardian` (Unknown ability)
- Gills: You can breathe underwater, but not on land.
- Wet Eyes: Your vision underwater is perfect.
- Aqua Affinity: You may break blocks underwater as others do on land.
- Fins: Your underwater speed is increased.
- Like Water: When underwater, you do not sink to the ground unless you want to.
- `origins:aquatic` (Unknown ability)
- `origins:conduit_power_on_land` (Unknown ability)
- `origins:air_from_potions` (Unknown ability)

## Origins-Monsters

### Creeper

`creeper` · (٭)

A remnant of an unexpected mistake, the Creeper blasts its way through its enemies.

**Ablities:**: 

- Creeper Ally: Creepers don't attack you!
- Explosive: You can sacrifice some of your health to create an explosion every 15 seconds.
- Afraid of Cats: You get nausea and weakness when around cats.
- Unwieldy: The way your hands are formed provide no way of holding a shield upright.

### Husk

`husk` · **UNCHOOSABLE** · (٭)

The Husks are Zombies that have become used to the much harsher heat of the deserts after staying there for a long period of time.

**Ablities:**: 

- Dark Sight: You can see in the dark when on land.
- Hunger: Anything you hit gets the Hunger effect.
- Undead Ally: Undead mobs don't attack you, unless you attack them first.
- `monsterorigins:undead` (Unknown ability)
- Zombie Slowness: Your undead body moves at a slower pace than humans.
- Warm Body: Your warm body conflicts with colder biomes, slowing you down.
- Zombie Hunger: Your constant hunger for flesh makes you exhaust quicker than a human.
- Zombie Touch: You zombify villagers instead of killing them.
- Terrifying Monster: Villagers are scared of you and refuse to trade with you.
- Metamorphosis: You transform into a Zombie if you're in water for too long.
- Carnivore: Your diet is restricted to meat, you can't eat vegetables.

### Piglin

`piglin` · (٭)

Piglins are strange creatures of unknown origin, who formed their own society and culture worshipping their love of gold in the harsh wastelands of the Nether.

**Ablities:**: 

- Gold Worshipper: Your adoration for gold unlocks its hidden power, making golden armor unbreakable and as strong as diamond.
- Bartering Master: You're brilliant at bartering after a lifetime of experience, every time you barter you get between 2 and 5 times as many valuables.
- Piglin Ally: Piglins don't attack you, unless you attack them first.
- Sniper: Your aim is more accurate than humans.
- Metamorphosis: You transform into a Zombified Piglin if you're out of the Nether for too long.
- Nether Inhabitant: Your natural spawn will be in the Nether.

### Skeleton

`skeleton` · (٭)

Reanimated bones of fallen archers, Skeletons have unlimited arrows and a perfect aim.

**Ablities:**: 

- Dark Sight: You can see in the dark when on land.
- Infinite Arrows: Arrows you shoot are not used up.
- Undead Ally: Undead mobs don't attack you, unless you attack them first.
- Sniper: Your aim is more accurate than humans.
- `monsterorigins:undead` (Unknown ability)
- Photoallergic: You burn in daylight.
- Skeletal Form: Your skeleton body means you have less health than humans.
- Poor Digestion: You can only hold half as much saturation as a human.
- `monsterorigins:metamorphosis_temperature` (Unknown ability)
- Metamorphosis: You transform into a Stray if you're in the cold for too long.

### Stray

`stray` · **UNCHOOSABLE** · (٭)

The Strays are frozen Skeletons that have found ways to harness the cold in order to slow down their enemies.

**Ablities:**: 

- Infinite Arrows: Arrows you shoot are not used up.
- `monsterorigins:freeze_immune` (Unknown ability)
- Undead Ally: Undead mobs don't attack you, unless you attack them first.
- Sniper: Your aim is more accurate than humans.
- Frozen Arrows: All arrows you shoot have the slowness effect.
- `monsterorigins:undead` (Unknown ability)
- Photoallergic: You burn in daylight.
- Skeletal Form: Your skeleton body means you have less health than humans.
- Cold Body: Your cold body conflicts with warmer biomes, slowing you down.
- Frozen Skin: You take double damage from all sources of fire.
- `monsterorigins:metamorphosis_temperature` (Unknown ability)
- Metamorphosis: You transform into a Skeleton if you're in a warm area for too long.

### Warden

`warden` · (٭٭٭)

Guardian of the Ancient Cities, the Warden relies on its senses of smell and hearing to find its target.

**Ablities:**: 

- Heightened Senses: You can see the outlines of nearby mobs, even through blocks.
- Blindness: You can't see anything further than a few blocks away, though you can see further with night vision.
- Powerful Swings: You deal twice as much damage as a normal player.
- Sonic Boom: Every 30 seconds you can launch a sonic boom by hitting the air with your hand.
- Tough Skin: You have double the health of a regular human.
- Sturdy Skin: Even without wearing armor, your skin provides natural protection.
- Unwieldy: The way your hands are formed provide no way of holding a shield upright.

### Wither Skeleton

`wither_skeleton` · (٭)

Locked away in the nether, over time Wither Skeletons have become resistant to fire.

**Ablities:**: 

- Wither: Anything you hit gets the Wither effect.
- Undead Ally: Undead mobs don't attack you, unless you attack them first.
- `monsterorigins:wither_immunity` (Unknown ability)
- `monsterorigins:undead` (Unknown ability)
- Skeletal Form: Your skeleton body means you have less health than humans.
- Poor Digestion: You can only hold half as much saturation as a human.
- Fire Immunity: You are immune to all types of fire damage.
- Nether Inhabitant: Your natural spawn will be in the Nether.

### Zombie

`zombie` · (٭)

Zombies are the corpses of fallen builders that have reawakened in the world for unknown reasons.

**Ablities:**: 

- Dark Sight: You can see in the dark when on land.
- Undead Ally: Undead mobs don't attack you, unless you attack them first.
- `monsterorigins:undead` (Unknown ability)
- Photoallergic: You burn in daylight.
- Zombie Slowness: Your undead body moves at a slower pace than humans.
- Zombie Hunger: Your constant hunger for flesh makes you exhaust quicker than a human.
- Zombie Touch: You zombify villagers instead of killing them.
- Terrifying Monster: Villagers are scared of you and refuse to trade with you.
- Metamorphosis: You transform into a Husk if you're in the desert for too long, and a Drowned if you're in the water for too long.
- `monsterorigins:metamorphosis_temperature` (Unknown ability)
- Carnivore: Your diet is restricted to meat, you can't eat vegetables.

### Zombified Piglin

`zombified_piglin` · **UNCHOOSABLE** · (٭)

Piglins are strange creatures of unknown origin, who formed their own society and culture worshipping their love of gold in the harsh wastelands of the Nether.

**Ablities:**: 

- Gold Desecration: Your evil corruption of gold unlocks a dark power, making golden weapons unbreakable and much stronger.
- Terrifying Armies: Nearby Zombified Piglins will attack anything that that attacks you or that you attack.
- Undead Ally: Undead mobs don't attack you, unless you attack them first.
- `monsterorigins:undead` (Unknown ability)
- Zombie Slowness: Your undead body moves at a slower pace than humans.
- Zombie Hunger: Your constant hunger for flesh makes you exhaust quicker than a human.
- Metamorphosis: You transform into a Piglin if you eat a golden apple when under the effect of a weakness potion.
- Nether Inhabitant: Your natural spawn will be in the Nether.
- Fire Immunity: You are immune to all types of fire damage.

### Drowned

`drowned` · **UNCHOOSABLE** · (٭)

The dwellers of the deep ocean, they wield their tridents and attack the living that dare cross the ocean.

**Ablities:**: 

- Trident Expert: You're a master of the trident, dealing +2 damage when you throw it, and +2 melee damage with it. You can also use channeling without thunder, and use riptide without rain/water at the price of extra durability.
- Water Vision: Your vision underwater is perfect.
- Guardian Ally: Guardians don't attack you!
- `monsterorigins:water_breathing` (Unknown ability)
- Fast Swimmer: Your underwater speed is increased.
- Water Combatant: You deal more damage while in water.
- `monsterorigins:undead` (Unknown ability)
- Photoallergic: You burn in daylight.
- Zombie Hunger: Your constant hunger for flesh makes you exhaust quicker than a human.
- Zombie Touch: You zombify villagers instead of killing them.
- Terrifying Monster: Villagers are scared of you and refuse to trade with you.
- Zombie Slowness: Your undead body moves at a slower pace than humans.
- Water Based: You're used to the water, so move much slower on land.
- Metamorphosis: You transform into a Zombie if you're in a warm area for too long.
- `monsterorigins:metamorphosis_temperature` (Unknown ability)
- Carnivore: Your diet is restricted to meat, you can't eat vegetables.

## All Abilities

### Scared of Gourds

`origins:pumpkin_hate`

You are afraid of pumpkins. For a good reason.

### Acrobatics

`origins:fall_immunity`

You never take fall damage, no matter from which height you fall.

### Weak Arms

`origins:weak_arms`

When not under the effect of a strength potion, you can only mine natural stone if there are at most 2 other natural stone blocks adjacent to it.

### Fragile

`origins:fragile`

You have 3 less hearts of health than humans.

### Featherweight

`origins:slow_falling`

You fall as gently to the ground as a feather would, unless you sneak.

### Fresh Air

`origins:fresh_air`

When sleeping, your bed needs to be at an altitude of at least %s blocks, so you can breathe fresh air.

### Vegetarian

`origins:vegetarian`

You can't digest any meat.

### Oviparous

`origins:lay_eggs`

Whenever you wake up in the morning, you will lay an egg.

### Unwieldy

`origins:no_shield`

The way your hands are formed provide no way of holding a shield upright.

### Master of Webs

`origins:master_of_webs`

You navigate cobweb perfectly, and are able to climb in them. When you hit an enemy in melee, they get stuck in cobweb for a while. Non-arthropods stuck in cobweb will be sensed by you. You are able to craft cobweb from string.

### Tailwind

`origins:tailwind`

You are a little bit quicker on foot than others.

### Climbing

`origins:climbing`

You are able to climb up any kind of wall, not just ladders.

### Carnivore

`origins:carnivore`

Your diet is restricted to meat, you can't eat vegetables.

### Gills

`origins:water_breathing`

You can breathe underwater, but not on land.

### Wet Eyes

`origins:water_vision`

Your vision underwater is perfect.

### Nocturnal

`origins:cat_vision`

You can slightly see in the dark when not in water.

### Nine Lives

`origins:nine_lives`

You have 1 less heart of health than humans.

### Photoallergic

`origins:burn_in_daylight`

You begin to burn in daylight if you are not invisible.

### Hydrophobia

`origins:water_vulnerability`

You receive damage over time while in contact with water.

### Invisibility

`origins:invisibility`

While phantomized, you are invisible.

### Teleportation

`origins:throw_ender_pearl`

Whenever you want, you may throw an ender pearl which deals no damage, allowing you to teleport.

### Fire Immunity

`origins:fire_immunity`

You are immune to all types of fire damage.

### Fins

`origins:swim_speed`

Your underwater speed is increased.

### Like Water

`origins:like_water`

When underwater, you do not sink to the ground unless you want to.

### Need for Mobility

`origins:light_armor`

You can not wear any heavy armor (armor with protection values higher than chainmail).

### Brittle Bones

`origins:more_kinetic_damage`

You take more damage from falling and flying into blocks.

### Hotblooded

`origins:hotblooded`

Due to your hot body, venoms burn up, making you immune to poison and hunger status effects.

### Burning Wrath

`origins:burning_wrath`

When on fire, you deal additional damage with your attacks.

### Strong Ankles

`origins:sprint_jump`

You are able to jump higher by jumping while sprinting.

### Aerial Combatant

`origins:aerial_combatant`

You deal substantially more damage while in Elytra flight.

### Winged

`origins:elytra`

You have Elytra wings without needing to equip any.

### Gift of the Winds

`origins:launch_into_air`

Every 30 seconds, you are able to launch about 20 blocks up into the air.

### Fast Metabolism

`origins:hunger_over_time`

Being phantomized causes you to become hungry.

### Large Appetite

`origins:more_exhaustion`

You exhaust much quicker than others, thus requiring you to eat more.

### Nether Inhabitant

`origins:nether_spawn`

Your natural spawn will be in the Nether.

### Claustrophobia

`origins:claustrophobia`

Being somewhere with a low ceiling for too long will weaken you and make you slower.

### Velvet Paws

`origins:velvet_paws`

Your footsteps don't cause any vibrations which could otherwise be picked up by nearby lifeforms.

### Aqua Affinity

`origins:aqua_affinity`

You may break blocks underwater as others do on land.

### Phasing

`origins:phasing`

While phantomized, you can walk through solid material, except Obsidian.

### Catlike Appearance

`origins:scare_creepers`

Creepers are scared of you and will only explode if you attack them first.

### Strong Arms

`origins:strong_arms`

You are strong enough to break natural stones without using a pickaxe.

### Hoarder

`origins:shulker_inventory`

You have access to an additional 9 slots of inventory, which keep the items on death.

### Sturdy Skin

`origins:natural_armor`

Even without wearing armor, your skin provides natural protection.

### Slender Body

`origins:extra_reach`

You can reach blocks and entities further away.

### Creeper Ally

`monsterorigins:creeper_ally`

Creepers don't attack you!

### Explosive

`monsterorigins:explosive`

You can sacrifice some of your health to create an explosion every 15 seconds.

### Afraid of Cats

`monsterorigins:fear_cats`

You get nausea and weakness when around cats.

### Metamorphosis

`monsterorigins:drowned_transform_into_zombie`

You transform into a Zombie if you're in a warm area for too long.

### Metamorphosis

`monsterorigins:husk_transform_into_zombie`

You transform into a Zombie if you're in water for too long.

### Metamorphosis

`monsterorigins:transform_into_husk_and_drowned`

You transform into a Husk if you're in the desert for too long, and a Drowned if you're in the water for too long.

### Metamorphosis

`monsterorigins:transform_into_stray`

You transform into a Stray if you're in the cold for too long.

### Metamorphosis

`monsterorigins:transform_into_skeleton`

You transform into a Skeleton if you're in a warm area for too long.

### Blindness

`monsterorigins:blindness`

You can't see anything further than a few blocks away, though you can see further with night vision.

### Heightened Senses

`monsterorigins:sense_movement`

You can see the outlines of nearby mobs, even through blocks.

### Tough Skin

`monsterorigins:double_health`

You have double the health of a regular human.

### Powerful Swings

`monsterorigins:double_damage`

You deal twice as much damage as a normal player.

### Sonic Boom

`monsterorigins:sonic_boom`

Every 30 seconds you can launch a sonic boom by hitting the air with your hand.

### Water Vision

`monsterorigins:water_vision`

Your vision underwater is perfect.

### Dark Sight

`monsterorigins:land_night_vision`

You can see in the dark when on land.

### Frozen Skin

`monsterorigins:double_fire_damage`

You take double damage from all sources of fire.

### Photoallergic

`monsterorigins:burn_in_day`

You burn in daylight.

### Trident Expert

`monsterorigins:trident_expert`

You're a master of the trident, dealing +2 damage when you throw it, and +2 melee damage with it. You can also use channeling without thunder, and use riptide without rain/water at the price of extra durability.

### Zombie Hunger

`monsterorigins:zombie_hunger`

Your constant hunger for flesh makes you exhaust quicker than a human.

### Poor Digestion

`monsterorigins:half_max_saturation`

You can only hold half as much saturation as a human.

### Guardian Ally

`monsterorigins:guardian_ally`

Guardians don't attack you!

### Water Combatant

`monsterorigins:water_combatant`

You deal more damage while in water.

### Undead Ally

`monsterorigins:undead_ally`

Undead mobs don't attack you, unless you attack them first.

### Wither

`monsterorigins:apply_wither_effect`

Anything you hit gets the Wither effect.

### Infinite Arrows

`monsterorigins:infinite_arrows`

Arrows you shoot are not used up.

### Frozen Arrows

`monsterorigins:slowness_arrows`

All arrows you shoot have the slowness effect.

### Hunger

`monsterorigins:apply_hunger_effect`

Anything you hit gets the Hunger effect.

### Skeletal Form

`monsterorigins:skeleton_body`

Your skeleton body means you have less health than humans.

### Zombie Slowness

`monsterorigins:slowness`

Your undead body moves at a slower pace than humans.

### Water Based

`monsterorigins:land_slowness`

You're used to the water, so move much slower on land.

### Fast Swimmer

`monsterorigins:swim_speed`

Your underwater speed is increased.

### Cold Body

`monsterorigins:heat_slowness`

Your cold body conflicts with warmer biomes, slowing you down.

### Sniper

`monsterorigins:better_aim`

Your aim is more accurate than humans.

### Warm Body

`monsterorigins:cold_slowness`

Your warm body conflicts with colder biomes, slowing you down.

### Zombie Touch

`monsterorigins:zombie_touch`

You zombify villagers instead of killing them.

### Terrifying Monster

`monsterorigins:scare_villagers`

Villagers are scared of you and refuse to trade with you.

### Metamorphosis

`monsterorigins:transform_into_zombified_piglin`

You transform into a Zombified Piglin if you're out of the Nether for too long.

### Metamorphosis

`monsterorigins:transform_into_piglin`

You transform into a Piglin if you eat a golden apple when under the effect of a weakness potion.

### Gold Worshipper

`monsterorigins:better_gold_armour`

Your adoration for gold unlocks its hidden power, making golden armor unbreakable and as strong as diamond.

### Gold Desecration

`monsterorigins:better_gold_weapons`

Your evil corruption of gold unlocks a dark power, making golden weapons unbreakable and much stronger.

### Terrifying Armies

`monsterorigins:zombified_piglin_allies`

Nearby Zombified Piglins will attack anything that that attacks you or that you attack.

### Bartering Master

`monsterorigins:super_bartering`

You're brilliant at bartering after a lifetime of experience, every time you barter you get between 2 and 5 times as many valuables.

### Piglin Ally

`monsterorigins:piglin_ally`

Piglins don't attack you, unless you attack them first.

### Small Bug

`moborigins:small_bug`

You have 2 less hearts of health than humans.

### Small Fox

`moborigins:small_fox`

You have 2 less hearts of health than humans.

### Arcane Totems

`moborigins:lower_totem_chance`

Totems have a %s% chance not to break on use.

### Snow Trail

`moborigins:snow_trail`

You leave a trail of snow.

### Stronger Snowballs

`moborigins:stronger_snowballs`

Snowballs you throw are packed with ice, and deal 1 damage!

### Bee Wings

`moborigins:bee_wings`

You can use your tiny bee wings to descend slower as an ability.

### Stinger

`moborigins:stinger`

When you punch someone with your fist, you poison them for a few seconds.

### Fungus Hunger

`moborigins:warped_fungus_eater`

You can eat warped fungus to recover some hunger, along with a small speed boost.

### Water Combatant

`moborigins:water_combatant`

You deal more damage while in water.

### Queen Bee

`moborigins:queen_bee`

When you collect honey, the bees won't try to attack you.

### Undead

`moborigins:undead`

You are undead, and burn in the daylight. You also take more damage from smite.

### Sly

`moborigins:sly`

You are quicker.

### Timid Creature

`moborigins:timid_creature`

Your speed increases when you are around more than 3 other players.

### Pillager Aligned

`moborigins:pillager_aligned`

Villagers don't like you, and pillagers like you!

### Small Weakness

`moborigins:small_weak`

When at less than 2 hearts, you deal almost no damage, but your attacks have stronger knockback!

### Rideable Creature

`moborigins:rideable_creature`

Other players can ride you!

### Guardian Ally

`moborigins:guardian_ally`

Guardians don't attack you!

### Surface Weakness

`moborigins:surface_weakness`

You are weakened while on land.

### Guardian Spikes

`moborigins:guardian_spikes`

Spikes that have a chance to damage attackers!

### Prismarine Skin

`moborigins:prismarine_skin`

Your skin is made of prismarine, and you get natural armor from it.

### Careful Gatherer

`moborigins:careful_gatherer`

Sweet Berry Bushes don't hurt you at all.

### Frigid Strength

`moborigins:frigid_strength`

Deal more damage in cold areas.

### Better Berries

`moborigins:better_berries`

Berries taste extra delicious to you!

### Wolf Body

`moborigins:wolf_body`

You have 2 less hearts of health than humans.

### Alpha Wolf

`moborigins:alpha_wolf`

Wolves you tame are stronger!

### Item Collector

`moborigins:item_collector`

You have a larger item pickup radius.

### Better Potions

`moborigins:better_potions`

You consume potions better than most, Potions will last longer when you drink them.

### Elder Magic

`moborigins:elder_magic`

You can cast a spell on nearby players to slow down their mining speed.

### Elder Spikes

`moborigins:elder_spikes`

Spikes that have a chance to damage attackers!

### Summon Fangs

`moborigins:summon_fangs`

You have the ability to summon fangs!

### Werewolf-like

`moborigins:full_moon`

During a full moon you get Faster, Stronger, and Healthier.

### Wolf Pack

`moborigins:wolf_pack`

When you are near at least 4 wolves you gain speed and attack damage.

### Zombie Hunger

`moborigins:zombie_hunger`

Your constant hunger for flesh makes you exhaust quicker than a human.

### Overheat

`moborigins:overheat`

You have a temperature bar that slowly begins to fill in hot biomes, and cool in other biomes.

### Melting

`moborigins:melting`

As your temperature bar fills up, you'll slowly begin to melt in hot biomes, losing health and speed.

### Howl

`moborigins:wolf_howl`

You can use the left click key when holding nothing to howl, and give speed and strength to nearby wolves and yourself.

### Trident Expert

`moborigins:trident_expert`

You're a master of the trident, dealing +2 damage when you throw it, and +2 melee damage with it. You can also use channeling without thunder, and use riptide without rain/water at the price of extra durability.

### Flower Power

`moborigins:flower_power`

When near multiple flowers, you gain regeneration.

### Bouncy

`moborigins:bouncy`

All blocks act like slime blocks.

### Lava Walker

`moborigins:lava_walk`

You have the ability to walk on lava source blocks! You are also quicker while walking on lava, and slower on land.

### Split Ability

`moborigins:split`

Turn your food points into a small slime to defend you!

### Perfect Potion

`moborigins:potion_action`

Get a random potion effect, based on the situation you are in.

### Dark Magic

`magicorigins:kill_boost`

Whenever you kill something, you absorb some of its health for up to 30 extra hearts.

### Cursed Power

`magicorigins:cursed_strikes`

Upon hitting something, it gains a negative effect for 15 seconds.

### Undead Ally

`magicorigins:undead_ally`

With your power over darkness, undead creatures will not attack you unprovoked.

### Magic Resistant

`magicorigins:no_magic`

Your dark magic repels regular forms of magic like potions and enchantments.

### Dark Aura

`magicorigins:dark_aura`

You emit a dark aura that makes villagers afraid of you.

### Alchemy

`magicorigins:gold_converter`

Right clicking on a Copper Block will turn it to Gold, and vice versa.

### Unskilled

`magicorigins:unskilled_warrior`

Your reliance on your magic has left you unskilled at melee combat.

### Hydrophobia

`magicorigins:weak_in_water`

Water saps your energy, leaving you with only 3 hearts of health.

### Hypnosis

`magicorigins:control_monsters`

Right clicking a monster will hypnotise it to target the last other thing you attacked.

### Confusion

`magicorigins:interactions_give_nausea`

Right clicking on a player will give them Nausea for 30 seconds.

### Potion Master

`magicorigins:potion_master`

Potions you drink and throw are much stronger.

### Spectral

`magicorigins:silent`

Your ghost-like nature prevents you being detected by Sculk Sensors when you move or eat.

### Ghostly

`magicorigins:invisible_when_still`

You can stand so still that you can't be seen.

### Spirit Strength

`magicorigins:spirit_strength`

You are stronger in the Nether, but weaker outside of it.

### Healing Focus

`magicorigins:regeneration_when_still`

When standing still you focus your energy to regenerate health.

### Final Shout

`magicorigins:spare_life`

You can survive death once every 10 minutes.

### Healing Touch

`magicorigins:interact_regeneration`

Right clicking on something will give it Regeneration II for 10 seconds.

### Lord of the Dead

`magicorigins:undead_commander`

Nearby undead monsters not targeting you will go after whatever you attack.

### Resurrection Spell

`magicorigins:bring_back_dead`

When you swing your fist, nearby dead players that have not yet respawned come back where they died.

### Shadow Form

`magicorigins:invisible_in_darkness`

In really dark places you enter Shadow Form, where nothing see or attack you.

### Dark Strength

`magicorigins:dark_strength`

You deal more damage in the dark, but less in the light.

### Creature of Darkness

`magicorigins:burn_in_light`

You catch fire in the presence of light, even in small amounts.

### Telekinetic Reach

`magicorigins:increased_reach`

You can reach things much further away than normal.

### Telekinesis

`magicorigins:telekinesis`

Items dropped from blocks and entities go straight into your inventory.

### Allay Master

`fantasyorigins:allay_master`

Your musical aura allows you to breed allays without playing music.

### Arrow Lord

`fantasyorigins:arrow_effect_booster`

Your connection to your bow and arrow enhances any potion effects placed on your arrows.

### Bardic Intuition

`fantasyorigins:bardic_intuition`

Your musical energy will sometimes cause a creeper to drop a music disc, even without a skeleton.

### Bow Burst

`fantasyorigins:bow_burst`

By casting a spell on any regular arrow, you can instantly shoot %s arrows at once using only one, but this disables your bow for %s seconds.

### Dragon's Breath

`fantasyorigins:breath_storer`

By right clicking using an empty bottle, you can store your own Dragon's Breath.

### Chime

`fantasyorigins:chime`

You can absorb the chime of amethyst shards to regenerate health.

### Double Health

`fantasyorigins:double_health`

As you're larger than humans, you have more health as your body protects you from damage.

### Dragon's Fireball

`fantasyorigins:dragon_fireball`

You can right click whilst holding a sword to launch a dragon's fireball, with a cooldown of %s seconds.

### Elegy

`fantasyorigins:elegy`

You become stronger when at less than 3 hearts.

### Crystal Healer

`fantasyorigins:end_crystal_healing`

You can regenerate health from nearby End Crystals.

### End Inhabitant

`fantasyorigins:end_boost`

Your natural habitat is the end, so you have more health and are stronger when you are there.

### Careful Miner

`fantasyorigins:fortune_increaser`

Your care and mastery in the art of extracting minerals results in a much higher yield from ores than other creatures.

### Piercing Shot

`fantasyorigins:increased_arrow_damage`

All arrows you shoot deal increased damage.

### Masterful Speed

`fantasyorigins:increased_arrow_speed`

Arrows you shoot are twice as fast than ones shot by a regular human.

### Heavy Blow

`fantasyorigins:heavy_blow`

Your attacks are stronger than humans, but you have a longer attack cooldown.

### Dashmaster

`fantasyorigins:increased_speed`

From years of training for race after race, you're much faster than any normal horse.

### Fast Miner

`fantasyorigins:infinite_haste`

You're well trained in mining, so are much faster than a regular human.

### Dark Eyes

`fantasyorigins:infinite_night_vision`

You can see in the dark after generations of evolution.

### Ocean Wish

`fantasyorigins:ocean_wish`

Your natural habitat is the ocean, so you're much weaker when you're not in the water.

### Iron Stomach

`fantasyorigins:magic_resistance`

You have an immunity to poison and harming potion effects.

### Moon's Blessing

`fantasyorigins:moon_strength`

You're a worshipper of the moon, so on nights with a full moon you're stronger than normal.

### Natural Armor

`fantasyorigins:natural_armor`

Your tough and strong body gives you some natural defense against attacks.

### Musically Attuned

`fantasyorigins:note_block_power`

You gain strength and speed when a nearby Note Block is played.

### Skilled Archer

`fantasyorigins:perfect_shot`

You've trained with a bow for many years, and you shoot your arrows perfectly straight.

### Half Horse

`fantasyorigins:permanent_horse`

You are half horse, half human.

### Clumsy Shot

`fantasyorigins:poor_shot`

Your big hands are clumsy with a bow, so your arrows are slow and not very accurate.

### Thick Skin

`fantasyorigins:strong_skin`

You've got naturally thicker skin than regular humans, so arrows do a lot less damage.

### Powerful Jump

`fantasyorigins:super_jump`

You've trained for your whole life, so can jump much higher than a regular horse.

### Ocean's Grace

`fantasyorigins:oceans_grace`

You are a part of the water, so you have extra health and deal extra damage when in water or rain.

### Vampiric Transformation

`fantasyorigins:vampiric_transformation`

You can transform other players into vampires by killing them.

### Daylight Sensitive

`fantasyorigins:daylight_sensitive`

Your greatest weakness is daylight, which causes you to burst into flames.

### Water Sensitive

`fantasyorigins:water_sensitive`

You are hurt by water as its current drains your power.

### Leeching

`fantasyorigins:leeching`

Upon killing a mob or player, you sap a portion of its health, healing you.

### Stronger

`fantasyorigins:stronger`

Your vampiric nature makes you stronger than a regular human, making your physical attacks deal far more damage.

### Undead Ally

`fantasyorigins:undead_ally`

As an undead monster, other undead creatures will not attack you unprovoked.

### Solid Stance

`fantasyorigins:solid_stance`

You are immune to knockback.

### Stiff

`fantasyorigins:stiff`

Your stone body makes it harder to move, causing you to walk slower.

### Heat Resistant

`fantasyorigins:fire_resistant`

Your stone skin makes you immune to fire.

### Potion Resistant

`fantasyorigins:potion_resistant`

Potions have no effect on you as your body is made of stone.

### Large Body

`fantasyorigins:large_body`

You're three blocks tall, taller than a regular human.

### Small Body

`fantasyorigins:small_body`

You're only one block tall, half as tall as a regular human.

### Call of the Sea

`squid1127:call_of_the_sea_display`

You can call a thunderstorm on command using /cots every so often.

### Poo

`squid1127:cocoa_display`

You will drop cocoa beans (poo) when sneaking

### Curl Up

`squid1127:curl_up_display`

You can curl up into a ball whilst sneaking, giving you resistance and slowness

### Frog Leap

`squid1127:frog_jump_display`

Double left-clicking while sneaking allows you to leap into the air.

### Wind Jump

`squid1127:wind_jump_display`

You release a wind charge when sneaking, allowing you to boost yourself into the air.

